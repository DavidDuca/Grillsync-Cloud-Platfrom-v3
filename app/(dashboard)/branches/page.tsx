      ...
      <div className="glass overflow-x-auto rounded-2xl">
        <table className="w-full min-w-[640px] text-sm">
          <thead className="border-b border-border text-left text-muted">
            <tr>{["Name","Address","Status","Last sync","Branch ID"].map((h) => <th key={h} className="px-4 py-3 font-medium whitespace-nowrap">{h}</th>)}</tr>
          </thead>
          <tbody>
            {branches.map((b) => (
        ...
        <Modal onClose={() => setNewCreds(null)} title="Branch created — save credentials">
          ...
          {["branchId","apiKey","apiSecret"].map((k) => (
            <div key={k} className="mb-2 flex items-center justify-between gap-2 rounded-md border border-border bg-panel2 p-3">
              ...
              <button onClick={() => navigator.clipboard.writeText(newCreds.branch[k])} className="shrink-0 rounded-md border border-border px-2 py-1 text-xs"><Copy className="h-3 w-3" /></button>
            </div>
          ))}
        </Modal>
      )}
